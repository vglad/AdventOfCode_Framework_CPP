...
namespace AoC {
    ...
    auto Day03::generate_data(int32_t x_pos, int32_t y_pos, Day03::Run run,
            ...
            -> std::unique_ptr<uMap> {
        auto const coord = std::make_pair(x_pos, y_pos);
        if (run == Run::First) {
            auto const data = std::make_pair(static_cast<int8_t>(Run::First), step_num);
    ...
    auto Day03::generate_wire_map(std::vector<std::string> const& path,
                                  Day03::Run                      run,
    ...
    auto Day03::get_min_distance(Day03::uMap const& u_map) -> int32_t {
        auto constexpr comp =
            [](std::pair<Coord , Data> const& a, std::pair<Coord , Data> const& b) {
                auto const & xa = a.first.first,  xb = b.first.first;
                auto const & ya = a.first.second, yb = b.first.second;
                return (std::abs(xa) + std::abs(ya))
                     < (std::abs(xb) + std::abs(yb));
        };
    ...
    void Day03::calculate_part1() {
            ...
            in_file)
        {
            ...
            in_file >> s;
            auto const path1 = tokenize_by_delim<inData_t>(s, ",");
            in_file >> s;
            auto const path2 = tokenize_by_delim<inData_t>(s, ",");
            in_file.close();
